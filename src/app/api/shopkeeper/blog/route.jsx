...
export async function GET(request) {
  try {
    ...
    const posts = await Blog.find({ authorId: tokenData.id }).sort({ createdAt: -1 }).lean();
...
export async function POST(request) {
  try {
    ...
    const post = await Blog.create({
      ...
      slug: makeSlug(title),
      authorName: tokenData.shopName || tokenData.userName || "Shopkeeper",
...
export async function PATCH(request) {
  try {
    ...
    const existing = await Blog.findOne({ _id: id, authorId: tokenData.id });
