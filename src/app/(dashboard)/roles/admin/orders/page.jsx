...
const NAV = [
  { label: "Overview",       href: "/roles/admin",           active: false },
  { label: "Sell Interests", href: "/roles/admin/interests", active: false },
  { label: "Orders",         href: "/roles/admin/orders",    active: true  },
];
  ...
  return (
    <>
          ...
          <div className="fade-in" style={{ marginBottom: 32, display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
            <div>
              ...
              <p style={{ color: "#64748b", marginTop: 6, fontSize: 14 }}>
                All guest checkout orders. Click any row to view details.
              </p>
            </div>
            {!fetching && (
              <div style={{ textAlign: "right" }}>
                <div style={{ fontSize: 24, fontWeight: 800, color: "#10b981", fontFamily: "'Instrument Serif', serif" }}>
                  Rs. {totalRevenue.toLocaleString()}
                </div>
                <div style={{ fontSize: 12, color: "#94a3b8", marginTop: 2 }}>
                  Revenue ({filter === "all" ? "all" : filter})
                </div>
              </div>
            )}
          </div>
                ...
                return (
                  <div key={o._id} className="row-item" onClick={() => setSelected(o)} style={{
                    ...
                    <div style={{ fontSize: 12, color: "#64748b", textTransform: "capitalize" }}>
                      {o.paymentMethod === "cod" ? "Cash on Delivery" : o.paymentMethod?.replace("_", " ")}
                    </div>
                    <span style={{
      ...
      {selected && (
        ...
        >
          <div
            style={{ background: "#fff", borderRadius: 16, width: "100%", maxWidth: 560, maxHeight: "85vh", overflow: "auto", boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}
          ...
          >
            ...
            <div style={{ padding: 24 }}>
              ...
              <Section title="Items">
                ...
                <div style={{ borderTop: "1px solid #f1f5f9", marginTop: 8, paddingTop: 8 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "#64748b", marginBottom: 4 }}>
                    <span>Subtotal</span><span>Rs. {selected.subtotal?.toLocaleString()}</span>
                  </div>
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "#64748b", marginBottom: 4 }}>
                    <span>Delivery</span>
                    <span style={{ color: selected.deliveryFee === 0 ? "#059669" : "#64748b" }}>
                      {selected.deliveryFee === 0 ? "FREE" : `Rs. ${selected.deliveryFee}`}
                    </span>
                  </div>
              ...
              {selected.notes && (
                <Section title="Notes">
                  <div style={{ fontSize: 13, color: "#64748b" }}>{selected.notes}</div>
                </Section>
              )}
                ...
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  {STATUS_FLOW.map((s) => {
                    ...
                    return (
                      <button
                        key={s}
                        disabled={updating || isCurrent || selected.status === "cancelled"}
                  ...
                  <button
                    disabled={updating || selected.status === "cancelled" || selected.status === "delivered"}
                    ...
                    style={{
                      ...
                      border: "none", opacity: updating ? 0.6 : 1,
                    }}
                  >
                    Cancelled
                  </button>
