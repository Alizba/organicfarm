...
const NAV = [
  ...
  { label: "Partners",         href: "/roles/admin/partners" },
];
...
export default function AdminPartnersPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError(e?.response?.data?.error || "Failed to save partner.");
    ...
    <div style={{ minHeight: "100vh", background: "#f8fafc", fontFamily: "system-ui, sans-serif" }}>
        ...
        <div style={{ display: "flex", gap: 32, overflowX: "auto" }}>
          {NAV.map((n) => (
            <a key={n.href} href={n.href} style={{
              padding: "16px 0", fontSize: 14, fontWeight: 600, whiteSpace: "nowrap",
              color: n.href === "/roles/admin/partners" ? "#0f172a" : "#94a3b8",
              borderBottom: n.href === "/roles/admin/partners" ? "2px solid #0f172a" : "2px solid transparent",
              textDecoration: "none",
            }}>{n.label}</a>
          ))}
        </div>
      ...
      <div style={{ padding: "40px" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 28 }}>
          <div>
            <h1 style={{ fontSize: 28, fontWeight: 700, color: "#0f172a" }}>Partners</h1>
            <p style={{ color: "#64748b", marginTop: 4, fontSize: 14 }}>Manage farm partners shown on the home page.</p>
          </div>
        ...
        {showForm && (
          <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 12, padding: 24, marginBottom: 24 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, alignItems: "start" }}>
              <div>
                <label style={{ fontSize: 11, fontWeight: 700, color: "#64748b", textTransform: "uppercase", display: "block", marginBottom: 6 }}>Partner Name *</label>
                <input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. FarmFresh Co."
                  style={{ width: "100%", padding: "10px 12px", fontSize: 13, border: "1px solid #e5e7eb", borderRadius: 8, outline: "none", boxSizing: "border-box" }} />
              </div>

              <div>
                <label style={{ fontSize: 11, fontWeight: 700, color: "#64748b", textTransform: "uppercase", display: "block", marginBottom: 6 }}>Logo Image *</label>
                {imagePreview ? (
                  <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    ...
                    <button onClick={() => { setImageFile(null); setImagePreview(null); if (fileRef.current) fileRef.current.value = ""; }}
                      style={{ fontSize: 12, color: "#dc2626", cursor: "pointer", background: "none", border: "none" }}>Remove</button>
                  </div>
                ) : (
                  <div onClick={() => fileRef.current?.click()}
                    style={{ border: "2px dashed #e5e7eb", borderRadius: 8, padding: "16px", textAlign: "center", cursor: "pointer", background: "#fafaf9" }}>
            ...
            <button onClick={handleSubmit} disabled={saving}
              style={{ marginTop: 18, background: "#0f172a", color: "#fff", border: "none", borderRadius: 8, padding: "10px 24px", fontSize: 13, fontWeight: 600, cursor: "pointer", opacity: saving ? 0.6 : 1 }}>
        ...
        {fetching ? (
          ...
          <div style={{ textAlign: "center", padding: 60, color: "#94a3b8", background: "#fff", borderRadius: 12, border: "1px solid #e5e7eb" }}>
            No partners yet. Click <strong>+ Add Partner</strong> to get started.
          </div>
        ) : (
            ...
            {partners.map((p) => (
              <div key={p._id} style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 12, padding: 20, display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
                ...
                <button onClick={() => handleDelete(p._id)}
                  style={{ background: "#fef2f2", color: "#dc2626", border: "none", borderRadius: 7, padding: "5px 14px", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>
                  Remove
                </button>
              </div>
            ))}
