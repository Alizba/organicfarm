  ...
  useEffect(() => {
    if (token.length > 0) {
      ...
      axios.post("/api/auth/verifyemail", { token })
        ...
        .catch((err) => { setError(true); console.log(err.response?.data); });
    }
  }, [token]);
    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-8">
      ...
      {verified && (
        <div className="text-center">
          ...
          <Link href="/login" className="text-green-800 underline font-medium">Go to Login</Link>
        </div>
      )}
      ...
      {error && (
        <div className="text-center">
          ...
          <Link href="/login" className="text-red-700 underline font-medium">Back to Login</Link>
        </div>
      )}
