...
const Navbar = () => {
  ...
  const NAV_LINKS = ["Home", "Shop", "Gallery", "Blog", "Contact"]
    ...
    <nav className="bg-white border-b border-gray-200 sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-4 flex items-center justify-between gap-8">
        ...
        <div className="flex items-center gap-4 md:gap-6 lg:gap-8">
          ...
          <button className="md:hidden p-2 text-gray-700" onClick={toggleMobileMenu} aria-label="Toggle menu">
      ...
      <div className={`fixed top-0 right-0 h-full w-80 bg-white shadow-2xl transform transition-transform duration-300 ease-in-out z-50 md:hidden ${
        ...
        <div className="flex items-center justify-between p-4 border-b border-gray-200">
          ...
          <button onClick={toggleMobileMenu} className="p-2 hover:bg-gray-100 rounded-md"><X size={24} /></button>
        </div>

        <div className="flex flex-col p-4 border-b border-gray-200">
          {NAV_LINKS.map((item) => (
            <Link key={item} href={getHref(item)} onClick={toggleMobileMenu}
              className="px-4 py-3 text-gray-700 font-medium hover:bg-gray-100 rounded-md transition-colors">
              {item}
            </Link>
          ))}
          <Link href="/sell" onClick={toggleMobileMenu}
            className="px-4 py-3 text-green-700 font-semibold hover:bg-green-50 rounded-md transition-colors">
            Want to Sell? ✨
          </Link>
        </div>

        <div className="flex flex-col p-4 gap-3">
          <button onClick={() => { handleCartClick(); toggleMobileMenu(); }}
            className="relative flex items-center gap-2 px-4 py-3 text-gray-700 hover:bg-gray-100 rounded-md">
            <ShoppingCart size={20} />
            <span>Cart</span>
            {cart.length > 0 && (
              <span className="ml-auto bg-red-500 text-white text-xs font-semibold px-1.5 py-0.5 rounded-full">{cart.length}</span>
            )}
          </button>

          {isLoggedIn ? (
            <>
              <Link href={user?.role === "admin" ? "/roles/admin" : "/roles/shopkeeper"} onClick={toggleMobileMenu}
            ...
            <Link href="/login" onClick={toggleMobileMenu}
              ...
              <User size={18} /><span>Staff Login</span>
            </Link>
          )}
        </div>
      </div>
